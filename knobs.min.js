...
(function (global, factory) {
	...
	/*! Color-Picker 0.11.0 MIT | https://github.com/yairEO/color-picker */
	...
	!function (t, e) {
	  ...
	  k.prototype = {
	    ...
	    getHSLA(t) {
	      let e;
	      if (t) return t + "" == "[object Object]" && Object.keys(t).join("").startsWith("hsl") ? t : (this.colorFormat = this.getColorFormat(t), t.indexOf("hsla") ? (t = r(t), t = h(t), e = c(t)) : (e = a(t), e = {
	        h: e[0],
	        s: e[1],
	        l: e[2],
	        a: e[3]
	...
	const position = props => {
	  ...
	  raf$1 = useRaf ? raf$1 : cb => cb();
	  prevPlacement = prevPlacement || [];
	  placement = (placement || ' ').split(' ').map((a, i) => a ? a : ['center', 'below'][i]);
	  offset = offset ? [offset[0] || 0, offset[1] || offset[0] || 0] : [0, 0];
	  if (ref.parentNode) {
	  ...
	  if (placement[0] == 'left') pos.x -= targetSize.w + offset[0];else if (placement[0] == 'right') pos.x += pos.w + offset[0];else pos.x -= targetSize.w / 2 - pos.w / 2;
	  if (placement[1] == 'above') pos.y -= targetSize.h + offset[1];else if (placement[1] == 'below') pos.y += pos.h + offset[1];else pos.y -= targetSize.h / 2 - pos.h / 2;
	  const overflow = {
	    top: pos.y < 0,
	  ...
	  const reposition = p => position({ ...props,
	    placement: p.join(' '),
	  ...
	  raf$1(() => {
	    ...
	    [['pos-left', pos.x],
	    ['pos-top', pos.y],
	    ['pos-target-width', targetSize.w], ['pos-target-height', targetSize.h], ['pos-ref-width', refRect.width || 0], ['pos-ref-height', refRect.height || 0], ['pos-ref-left', refRect.x], ['pos-ref-top', refRect.y], ['window-scroll-y', window.scrollY], ['window-scroll-x', window.scrollX]].forEach(([k, v]) => target.style.setProperty('--' + k, Math.round(v)));
	  });
	  return {
	    pos,
	    placement
...
`;
  ...
  `;
	...
	function onChange(e, ignoreSimilar) {
	  ...
	  if (knobData.type === 'color') extraData.hsla = colorPicker.exports.CSStoHSLA(colorPicker.exports.changeColorFormat(knobData.value, 'HSL'));
	...
	Knobs.prototype = {
	  ...
	  updateDOM({
	    ...
	    if (cssVarUnit && cssVarUnit[0] != '-') vars[0][1] += cssVarUnit || '';
	    if (!isToggled || type == 'checkbox' && knobInput && !knobInput.checked) action = 'removeProperty';
	    if (type == 'color' && cssVarsHSLA) {
	      const hsla = colorPicker.exports.CSStoHSLA(colorPicker.exports.changeColorFormat(value, 'HSL'));
	      vars.push([`${cssVarName}-h`, hsla[0]], [`${cssVarName}-s`, hsla[1]], [`${cssVarName}-l`, hsla[2]], [`${cssVarName}-a`, hsla[3]]);
	    }
	    if (Object.prototype.toString.call(targetElms).includes("Element")) targetElms = [targetElms];
	    if (targetElms && targetElms.length && value !== undefined && cssVarName) for (let elm of targetElms) for (let [prop, value] of vars) elm.style[action](`--${prop}`, value);
	  },
