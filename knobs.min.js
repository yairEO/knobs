	...
	function cloneKnobs (knobs, persistedData) {
	  return knobs.map(k => {
	    if (k && k.type) {
	      ...
	      if (!this.settings.knobsToggle && k.isToggled === false) delete k.isToggled;
	      if (persistedData) {
	        if (k.type == 'checkbox') {
...
`;
  ...
  `;
	...
	function bindEvents() {
	  ...
	  [['scope', 'click'], ['form', 'change'], ['form', 'input'], ['form', 'reset'], ['form', 'submit'], ['form', 'focusin'], ['form', 'transitionend'], ['form', 'transitionstart'], ['scope', 'wheel'], ['mainToggler', 'change', this.eventsRefs.mainToggler]].forEach(([elm, event, cb]) => this.DOM[elm] && this.DOM[elm].addEventListener(event, cb || this.eventsRefs[event].bind(this), {
	    passive: false
	  }));
	...
	function onInput(e) {
	  ...
	  this.setKnobDataByName(e.target.name, isCheckbox ? {
	    checked
	  } : {
	    value
	  });
	  if (value != undefined && label)
	    this.getSetPersistedData({
	      [label]: isCheckbox ? [inputElm.checked, value] : value
	    });
	}
	function onChange(e) {
	...
	function onClick(e) {
	  ...
	  if (target.dataset.type == 'color') setTimeout(_ => this.toggleColorPicker(target), 100);
	}
	...
	Knobs.prototype = {
	  ...
	  getKnobValueFromCSSVar(data) {
	    ...
	    if (!("value" in data) && data.cssVar && data.cssVar.length) {
	      ...
	      if (isNaN(value)) console.warn("@yaireo/knobs -", "Unable to parse variable value:", data.cssVar[0]);
	  ...
	  toggleColorPicker(inputElm) {
	    ...
	    cPicker = cPicker || new colorPicker_min({
	      color: value,
	      className: 'hidden',
	      swatches: [],
