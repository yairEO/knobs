...
(function (global, factory) {
	...
	!function (t, e) {
	  ...
	  function m(t) {
	    ...
	    this.DOM.scope.contains(t.target) && (t.target.value = Math.min(Math.max(+e + i, 0), +s), w.call(this, t));
	  }
	  function g(t) {
	  ...
	  var _ = Object.freeze({
	    __proto__: null,
	    bindEvents: function () {
	      [["scope", "input", w], ["scope", "change", f], ["scope", "click", C], ["value", "change", b]].forEach(([t, e, s]) => this.DOM[t].addEventListener(e, s.bind(this))), window.addEventListener("wheel", m.bind(this)), window.addEventListener("storage", S.bind(this)), this.settings.onClickOutside && (document.body.addEventListener("click", v.bind(this)), window.addEventListener("keydown", g.bind(this)));
	    }
	  });
	  function O() {
	  ...
	  const $ = "@yaireo/color-picker/swatches";
	  var y = Object.freeze({
	    __proto__: null,
	    getSetGlobalSwatches: function (t) {
	      const e = this.settings.swatchesLocalStorage,
	            s = "string" == typeof e ? e : "";
	      return e && t && (localStorage.setItem($ + s, t), dispatchEvent(new Event("storage"))), localStorage[$ + s]?.split(",").filter(String) || [];
	    },
	  ...
	  function M(t) {
	    ...
	    this.DOM = {}, this.sharedSwatches = this.getSetGlobalSwatches(), this.initialSwatches = o || [], this.swatches = o && this.sharedSwatches.concat(this.initialSwatches), this.setColor(this.getHSLA(this.changeColorFormat(s, i))), this.history = O.call(this), this.init();
	  }
	  return M.prototype = {
	    templates: p,
	    ...y,
	...
	function cloneKnobs (knobs, persistedData) {
	  return knobs.map(knob => {
	    if (knob && knob.type) {
	      ...
	      if (persistedData) knob.value = persistedData[knob.label] ?? knob.originalValue;
	      if (knob.type == 'range') {
	        knob.value = +knob.value;
	        knob.originalValue = +knob.originalValue;
	      }
	    }
	    return knob.cssVar ? { ...knob,
	      cssVar: [...knob.cssVar]
	    } : isObject(knob)
	    ? { ...knob
	    } : knob;
...
`;
  ...
  `;
	...
	function onInput(e) {
	  ...
	  if (inputelm.value != undefined) this.getSetPersistedData({
	    ...
	    (knobsData || this._knobs).forEach(d => {
	      ...
	      var isCheckbox = d.type == 'checkbox',
	        ...
	        target: inputElm
	          ...
	          resetTitle;
	      if (isCheckbox) resetTitle = inputElm.checked = !!d.checked;else resetTitle = inputElm.value = d[vKey];
